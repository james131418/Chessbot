...
class PositionUpdater():
    ...
    def advance(self, name, move):
		...
		if move[2:4] in self.right_arm_region:
                ...
                print "open"
                self.hand_planner.move_hand_interface(prev_pos.x, prev_pos.y, prev_pos.z + 0.0137 - 0.93, arm)
                print "down"
                ...
                print "approach"
                self.hand_planner.move_hand_interface(prev_pos.x + delta_x, prev_pos.y + delta_y, prev_pos.z + 0.014 - 0.93, arm)
                print "down"
                self.hand_planner.gripper_open()
                print "open"
                self.hand_planner.move_hand_interface(prev_pos.x + delta_x, prev_pos.y + delta_y, prev_pos.z + self.offset, arm)
                ...
                if arm == 'left':
                    self.hand_planner.move_hand_interface(0.4, 0.5, 0.1, arm)
                if arm == 'right':
                    self.hand_planner.move_hand_interface(0.4, -0.5, 0.1, arm)
    ...
    def init_pose(self):
        self.hand_planner.move_hand_interface(0.4, 0.5, 0.1, 'left')
        self.hand_planner.move_hand_interface(0.4, -0.5, 0.1, 'right')
